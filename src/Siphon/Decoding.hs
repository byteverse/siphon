...
import Siphon.Internal (comma, row)
import Siphon.Types
 ...
 where
  checkedRunWithRow rowIx v =
    let vlen = Vector.length v
     in if vlen /= requiredLength
          then
            Left $
              DecolonnadeRowError rowIx $
...
prettyError toStr (DecolonnadeRowError ix e) =
  unlines $
    ("Decolonnade error on line " ++ show (ix + 1) ++ " of file.")
      : ("Error Category: " ++ descr)
      : map ("  " ++) errDescrs
 where
  (descr, errDescrs) = prettyRowError toStr e
...
prettyRowError toStr x = case x of
  ...
  RowErrorMinSize reqLen actualLen ->
    (,)
      "Row Min Length"
      [ "Expected the row to have at least " ++ show reqLen ++ " cells."
      , "The row only has " ++ show actualLen ++ " cells."
      ]
  RowErrorMalformed enc ->
    (,)
      "Text Decolonnade"
...
prettyHeadingErrors conv (HeadingErrors missing duplicates) =
  concat
